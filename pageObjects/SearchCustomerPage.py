class SearchCustomer():
    ...
    def searchCustomerByEmail(self):

        emailid=self.driver.find_element_by_xpath(r"//tbody//tr//td[2]").text

        return emailid
    # print(type(searchCustomerByEmail()))
    def searchCustomerByName(self,Name):
        flag=False
        for r in range(1,self.getNoOfRows()+1):
          table=self.driver.find_element_by_xpath(self.table_xpath)
          name=self.driver.find_element_by_xpath("//table[@id='customers-grid']/tbody/tr["+str(r)+"]/td[3]").text
          if name == Name:
              flag = True
              break
        return